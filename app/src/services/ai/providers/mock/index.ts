    ...
    TradeAnalysisProvider
{
  ...
  async analyzeImage(imageUrl: string, prompt: string, options?: AIRequestOptions): Promise<AIResponse<string>> {
    ...
    return {
      data: `Based on the image analysis: ${prompt}`,
      provider: "mock",
      model: "mock-vision",
      processingTimeMs,
      timestamp: new Date().toISOString(),
  ...
  async extractText(imageUrl: string, options?: AIRequestOptions): Promise<AIResponse<string>> {
    ...
    return {
      data: "EURUSD Buy Entry: 1.08950 | Exit: 1.09120 | Profit: +85.00",
      provider: "mock",
      model: "mock-vision",
      processingTimeMs,
      timestamp: new Date().toISOString(),
  ...
  async summarizeTrades(tradeIds: string[], options?: AIRequestOptions): Promise<AIResponse<string>> {
    ...
    return {
      data: `Analysis of ${tradeIds.length} trades: Win rate 58%, avg R:R 1:2.1. Good trend-following performance.`,
      provider: "mock",
      model: "mock-gpt-4",
      processingTimeMs,
      timestamp: new Date().toISOString(),
  ...
  async chat(message: string, sessionId?: string, options?: AIRequestOptions): Promise<AIResponse<CoachMessage>> {
    ...
    const responses = [
      "That's a solid observation. Consider checking the volume profile to confirm strength.",
      "You're right to be cautious. The risk-reward isn't ideal here. Waiting for a pullback may be better.",
      "Looking at your recent trades, you've improved significantly in patience. Keep following your checklist.",
      "This pattern has a 72% success rate when volume confirms the breakout. Make sure to check that.",
      "Consider reviewing your journal to see which setups are working best for you.",
    ];
