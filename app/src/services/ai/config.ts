  ...
  return [
    {
      ...
      capabilities: [
        "chat", "vision", "ocr", "summary", "coaching",
        "transcription", "chart-analysis", "trade-analysis",
      ],
      priority: 0,
...
export function getProviderConfig(name: AIProviderName): AIProviderConfig | undefined {
  return getDefaultProviderConfigs().find((c) => c.name === name);
}
...
export function hasRealProviderConfigured(): boolean {
  return getDefaultProviderConfigs().some(
    (c) => c.name !== "mock" && c.enabled && c.apiKey
  );
}
